  ...
  <div style={{ marginBottom: 14 }}>
    ...
    <input
      style={{
        ...
        border: "1px solid #2a2a2a", borderRadius: 6, padding: "10px 14px",
        color: "#eee", fontSize: 14, outline: "none", transition: "border 0.2s",
      }}
...
const Btn = ({ children, loading, variant = "primary", ...props }) => (
  <button
    style={{
      ...
      fontWeight: 600, letterSpacing: "0.06em", cursor: loading ? "not-allowed" : "pointer",
      border: variant === "ghost" ? "1px solid #2a2a2a" : "none",
      background: variant === "ghost" ? "transparent" : loading ? "#3a3aaa" : "#5a5aff",
...
const Notice = ({ msg, type }) => msg ? (
  <div style={{
    ...
    background: type === "error" ? "#2a0d0d" : "#0d1f0d",
    border: `1px solid ${type === "error" ? "#5a1a1a" : "#1a4a1a"}`,
    color: type === "error" ? "#ff7a7a" : "#7aff7a",
  }}>{msg}</div>
) : null;
    ...
    <div style={{ width: 560 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
        ...
        <button onClick={onLogout} style={{
          ...
          color: "#666", padding: "6px 14px", fontSize: 12, cursor: "pointer",
      ...
      <div
        ...
        onDrop={onDrop}
        style={{
          border: `2px dashed ${dragging ? "#5a5aff" : "#222"}`,
          borderRadius: 10, padding: "40px 20px", textAlign: "center",
          marginBottom: 20, transition: "border 0.2s, background 0.2s",
          background: dragging ? "#0d0d2a" : "#0a0a0a", cursor: "pointer",
          position: "relative",
        }}
      ...
      >
        ...
        {uploading ? (
          ...
          <>
            ...
            <div style={{ color: "#555", fontSize: 13 }}>Drop AV1 video or click to browse</div>
          </>
        )}
      </div>
      ...
      {frame && (
        <div style={{ marginBottom: 20 }}>
          ...
          <img src={frame} alt="Last frame" style={{
            width: "100%", borderRadius: 8, border: "1px solid #1e1e1e", display: "block",
          }} />
        </div>
      )}
      ...
      <button onClick={loadVideos} style={{
        ...
        color: "#888", padding: "9px 18px", fontSize: 12, cursor: "pointer",
        letterSpacing: "0.06em", transition: "background 0.2s",
      }}>Load video list</button>
        ...
        <div style={{ marginTop: 16 }}>
          {Object.entries(videos).map(([arch, vids]) => (
            <div key={arch} style={{ marginBottom: 12 }}>
              ...
              {vids.map(v => (
                <div key={v} style={{
                  padding: "8px 12px", background: "#0d0d0d", border: "1px solid #1e1e1e",
                  borderRadius: 5, fontSize: 13, color: "#ccc", marginBottom: 4,
                }}>{v}</div>
              ))}
            </div>
          ))}
